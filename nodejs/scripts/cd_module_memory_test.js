...
// ************************
// メイン関数
export function cdMain(loopCount) {
  // ファイルパスを指定
  ...
  CdModuleFactory().then((CdModule) => { 
	...
	fs.readFile(linkShapePath, 'utf8', (errShapes, linkShapesData) => {
	  ...
	  try {
		...
		if (printFlag && collisionPairs.length > 0) {
		...
		if (printFlag && resultPairs.size() > 0)
		...
		if (printFlag && resultPairs.size() > 0) console.log("res: ", res);
		resultPairs.delete();
	      }
	    }
	  } catch (e) {
